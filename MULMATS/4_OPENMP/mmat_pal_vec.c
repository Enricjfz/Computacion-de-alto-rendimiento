 ...
 *
 * EJECUCION:  ./ejecutable [dimension_matrices,Imprimir(1)]
 *******************************************************/
...
void Mult(float *A, float *B, float *C, int fA, int cA, int cB) {
   ...
   #pragma omp parallel shared(cA,fA,cB)
   {
     #pragma omp for simd private(r,i,k,j)
     for (i=0; i<fA; i++)
        for (k=0; k<cA; k++) {
          r = A[i*cA+k];
          for (j=0; j<cB; j++)
...
int main(int argc, char **argv){
    if (argc <5) {
	printf("Uso: Ejecutable n_filas(A) n_columnas(A) n_columnas(B) Imprimir(1)\n");
	return(0);
    }
    ...
    int imprime = atoi(argv[4]); // Imprimir (1) algunos elementos de A,B y C
   ...
   // Se reserva memoria e inicializa a 0 las matrices A, B y C.
    ...
    printf("\nDuracion total de Mult: %f segundos\n\n", segundos);

    if (imprime==1){ //Se muestran algunos elementos por pantalla
